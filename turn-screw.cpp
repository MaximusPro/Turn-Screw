...
std::pair<int, int>FindTemplateWord(std::string &Line)
{
    ...
    {
        if(Line[i] == '#' && Indexs.first == 0){Indexs.first = i;}
        else if(Line[i] == '#' && Indexs.first != 0){Indexs.second = i;}
...
{
    //std::string Command;
    for(std::string Command: Commands){if(Arg == Command) return true;}
    return false;
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if(argc == 2 || argc%2 == 0){std::cout << "ERROR: You have mistakes in arguments!" << std::endl; return 2;}
    //std::cout << "Hello!\n";
    ...
    if(exists(PathToTemplates))
    {
        ...
        else FullPathProjectPlusDirVSCode = MainArguments.FullPathProject;
        ...
        while(getline(InFileLaunch, Line))
        {
            ...
            else {OutFileLaunch << Line << std::endl;}
        }
        InFileLaunch.close();
        OutFileLaunch.close();
        ...
        while(getline(InTasksFile, Line))
        {
            ...
            else {OutTasksFile << Line << std::endl;}
        }
        InTasksFile.close();
        OutTasksFile.close();
    }
    ...
    else {std::cout << "Directory with templates is not exist!\n";return -1;}
